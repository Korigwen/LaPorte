...
public class OSC : MonoBehaviour
{
    ...
    public CONNECTION_TYPE connectionType = CONNECTION_TYPE.UDP;

    public int inPort = 6969;
    public string outIP = "127.0.0.1";
    [ConditionalField(nameof(connectionType), false, CONNECTION_TYPE.UDP)] public int outPort = 6161;
    ...
    void Awake()
    {
        ...
        switch (connectionType)
        {
            case CONNECTION_TYPE.UDP:
                ...
                break;
            case CONNECTION_TYPE.WEB_SOCKET:
                OscPacketIO = new WSPacketIO(outIP, outPort, inPort);
                break;
        }
        ...
        this.SetAllMessageHandler((OscMessage message) =>
        {
            if (Debugging)
            {
                Debug.Log(message);
